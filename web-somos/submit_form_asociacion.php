...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $direccion = htmlspecialchars(substr(filter_var($_POST["direccion"], FILTER_SANITIZE_STRING), 0, 1000), ENT_QUOTES, 'UTF-8');
    $phone = htmlspecialchars(substr(filter_var($_POST["phone"], FILTER_SANITIZE_STRING), 0, 1000), ENT_QUOTES, 'UTF-8');

    // Validación de campos obligatorios y formato de correo electrónico
    if (empty($nombre) || empty($email) || empty($phone) || empty($direccion) || empty($mensaje)) {
